...
| SPLUNK_DEPLOYER_URL | One Splunk Enterprise deployer host (network alias). Optional; required only when SHC deployer functionality is used. | no | no | no |
...
| SPLUNK_KVSTORE_DEFAULT_TYPE | Configures `[kvstore] defaultKVStoreType` in `server.conf` when explicitly set. Allowed values are `cohosted`, `external`, and `local`; Ansible writes the setting only for Splunk Enterprise 10.6.0 or later. When set to `cohosted` or `external`, Ansible also writes `[kvstore] postgresMigrateOnStartup=true`; when set to `local`, it writes `[kvstore] postgresMigrateOnStartup=false`. Older Splunk versions skip the default KV Store type configuration. SOK injects `external` for CMP-K workloads configured with `spec.kvServiceRef`. | no | no | no |
...
| SPLUNK_NOAH_ADVERTISED_ADDR | The address a Noah **indexer** advertises to the Noah service, as `<scheme>://<host>:<port>`. Search heads and deployers do not advertise an address, and this value is not resolved for them. Takes precedence over a defaults-file `splunk.noah_advertised_addr`; when neither is set the address is derived as `<scheme>://<POD_NAME>.<SPLUNK_HEADLESS_SERVICE_NAME>.<POD_NAMESPACE>.svc.<CLUSTER_DOMAIN>:<SPLUNK_SVC_PORT>`, where the scheme follows the splunkd management endpoint (`SPLUNK_CERT_PREFIX`, `SPLUNKD_SSL_ENABLE`, `splunk.ssl.enable`). Provisioning fails before splunkd starts if the value is unusable — whether supplied or derived — or if derivation is required and any of `POD_NAME`, `POD_NAMESPACE` or `SPLUNK_HEADLESS_SERVICE_NAME` is absent. | no | no | no |
